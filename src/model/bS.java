...
public class bS {
	...
	/*@ requires (found() == false) && ( ar().getIndex(pivot()) < value()) && ( left() != right())
	  ...
	  @ ensures (left() == \old(pivot() +1)) && (right() == \old(right())) 
	  @ && (found() == (\old(ar().getIndex( (pivot() +1 + right()) / 2)))  == \old(value())) 
	  @ && (pivot() == \old( (pivot() +1 + right()) / 2) ) 
	  @ && ( (\old(ar().getIndex( (pivot() +1 + right()) / 2)))  == \old(value())  ==> toRetrun() == \old((pivot() +1 + right()) / 2) ) 
	  @ && ( (\old(ar().getIndex( (pivot() +1 + right()) / 2)))  != \old(value())  ==> toRetrun() == -1 ) ;
	  @*/
	public void  searchRight(){
		if ( ( the_found == false) && ( ar().getIndex(the_pivot) < the_value) && ( the_left != the_right)
			...
			the_left = the_pivot +1 ;
			
			if ( ar().getIndex((the_pivot +1 + the_right) / 2) == the_value ){
				the_found = true ;
			}else{the_found = false ;}
			
			if( ar().getIndex((the_pivot +1 + the_right) / 2) == value() ){
				the_return = (the_pivot +1 + the_right) / 2 ;
			}else{ the_return =  -1;}
		...
		if(the_left == the_right){the_found = true ;}
